# RFC-0012: Capability-based filesystem confinement
...
Add `cap-std` and open the configured root once as a `cap_std::fs::Dir`.
Native reads, writes, deletes, existence checks, renames, and directory walks
operate relative to that capability. Temporary files and their final rename
